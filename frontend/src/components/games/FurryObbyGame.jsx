...
function makeSignTexture(text) {
  ...
  g.addColorStop(0,"rgba(255,120,210,0.97)");
  g.addColorStop(1,"rgba(150,60,220,0.95)");
  ctx.fillStyle=g;
  ...
  ctx.strokeStyle="rgba(50,0,70,0.95)"; ctx.lineWidth=12;
...
// ─── BACKGROUND FURRY CHAOS ──────────────────────────────────────────────────
function makeBgPawPrint(scene, cx, cy, cz, scale, color) {
  var g=new THREE.Group();
  var mat=new THREE.MeshBasicMaterial({color:color||"#ff9de6",side:THREE.DoubleSide,transparent:true,opacity:0.55});
  // main pad (flat disc)
  var pad=new THREE.Mesh(new THREE.CircleGeometry(1,20),mat);
  g.add(pad);
  var toePos=[[-1.1,1.3],[-0.4,1.7],[0.4,1.7],[1.1,1.3]];
  for (var ti=0;ti<toePos.length;ti++) {
    var toe=new THREE.Mesh(new THREE.CircleGeometry(0.55,16),mat);
    toe.position.set(toePos[ti][0],toePos[ti][1],0);
    g.add(toe);
  }
  g.scale.set(scale,scale,scale);
  g.position.set(cx,cy,cz);
  // always face camera (billboard) — handled manually in animate
  g.userData={billboard:true};
  scene.add(g);
  return g;
}

function makeBgFurBall(scene, cx, cy, cz, r, color) {
  // cluster of overlapping spheres = fluffy look
...
function makeBgHeart(scene, cx, cy, cz, scale, color) {
  ...
  var g=new THREE.Group();
  var mat=new THREE.MeshBasicMaterial({color:color||"#ff4fc3",transparent:true,opacity:0.45});
...
function buildBackground(scene, bgObjects) {
  // Scatter furry bg elements along the entire course length (~800 Z units)
  var bgPal=["#ff9de6","#ff4fc3","#a855f7","#ffe18f","#9ee7ff","#b8ff9e","#ffb07c","#ff6699"];
  ...
  for (var zi=0;zi>-800;zi-=18) {
    ...
    var obj;
    if (type===0) obj=makeBgPawPrint(scene,cx,cy,cz,rng(4,10),col);
    else if (type===1) obj=makeBgFurBall(scene,cx,cy,cz,rng(3,8),col);
  ...
  // Big floating text signs in the sky — furry cringe
  var skyMessages=[
    "UWU","OWO","RAWR XD","NYA~","*POUNCES ON U*",
    "HOWLS INTERNALLY","BARK","FURSONA LOADING...","RAWR",
    "WOOF WOOF","YIFF","HEWWO!!","SOWWY >W<","hehe xd",
  ];
  ...
  // Enormous fur-ball clusters very far out
  var deepCols=["#ff9de6","#a855f7","#9ee7ff","#ffe18f"];
...
function makeFursuitHead(parent, hx, hy, hz, color) {
  color=color||"#ff9de6";
  var g=new THREE.Group();
  var fur=new THREE.MeshStandardMaterial({color:color,roughness:0.9});
  var snoutM=new THREE.MeshStandardMaterial({color:"#fff8f0",roughness:0.85});
  var pinkM=new THREE.MeshStandardMaterial({color:"#ff80c0",roughness:0.8});
  var darkM=new THREE.MeshStandardMaterial({color:"#222",roughness:0.4});
  var eyeM=new THREE.MeshStandardMaterial({color:"#1a0a2e",roughness:0.3});
  var shineM=new THREE.MeshBasicMaterial({color:"#fff"});

  var head=new THREE.Mesh(new THREE.SphereGeometry(0.8,18,14),fur); g.add(head);

  var earOffsets=[[-0.46,0.72,0],[0.46,0.72,0]];
  for (var ei=0;ei<earOffsets.length;ei++) {
    var eo=earOffsets[ei];
    var ear=new THREE.Mesh(new THREE.ConeGeometry(0.28,0.55,10),fur);
    ear.position.set(eo[0],eo[1],eo[2]); g.add(ear);
    var inner=new THREE.Mesh(new THREE.ConeGeometry(0.14,0.38,10),pinkM);
    inner.position.set(eo[0],eo[1],eo[2]+0.04); g.add(inner);
  }

  var snout=new THREE.Mesh(new THREE.SphereGeometry(0.34,14,10),snoutM);
  snout.position.set(0,-0.12,0.68); snout.scale.y=0.7; g.add(snout);

  var nose=new THREE.Mesh(new THREE.SphereGeometry(0.1,10,8),darkM);
  nose.position.set(0,0.02,0.99); g.add(nose);

  var eyeOff=[[-0.3,0.18,0.72],[0.3,0.18,0.72]];
  for (var ii=0;ii<eyeOff.length;ii++) {
    var eof=eyeOff[ii];
    var eye=new THREE.Mesh(new THREE.SphereGeometry(0.12,10,8),eyeM);
    eye.position.set(eof[0],eof[1],eof[2]); g.add(eye);
    var shine=new THREE.Mesh(new THREE.SphereGeometry(0.05,8,6),shineM);
    shine.position.set(eof[0]+0.04,eof[1]+0.04,eof[2]+0.06); g.add(shine);
  }
...
function makePawPrint(scene, cx, cy, cz, rot) {
  rot=rot||0;
  var g=new THREE.Group();
  var m=new THREE.MeshStandardMaterial({color:"#ff9de6",roughness:0.7});
  ...
  for (var ti=0;ti<toes.length;ti++) {
    var toe=new THREE.Mesh(new THREE.CylinderGeometry(0.18,0.18,0.08,10),m);
    toe.position.set(toes[ti][0],toes[ti][1],toes[ti][2]); g.add(toe);
  }
...
function makeTailSpinner(scene, hazards, opts) {
  ...
  var tipM=new THREE.MeshStandardMaterial({color:"#fff",emissive:"#ffe8fa",emissiveIntensity:0.22,roughness:0.9});
...
function buildWorld(scene, objects, hazards) {
  ...
  x=0; y+=0.1;
  cpBox(scene,objects,[x,y,z],2,"CHECKPOINT! NYA~ >w<");
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],3,"RAWR! CHECKPOINT 3 GET!");
  z-=12;
  ...
  makeSign(scene,"DODGE THE SPINNY TAIL XD",0,y+3.4,z-4,5.2,1.1);
  z-=14; y+=0.2;
  ...
  x=0;
  cpBox(scene,objects,[x,y,z],4,"UWU SECTION 3 CLEAR! OwO");
  ...
  makeBox(scene,objects,{pos:[0,y,z],size:[7,0.55,7],color:"#a855f7",emissive:"#5b21b6",type:"bounce",level:4,bounce:14});
  makeSign(scene,"BOINGY PAW PAD OwO",0,y+3,z-2.8,4.5,1.1);
  z-=9; y+=3.3;
  makeBox(scene,objects,{pos:[0,y,z],size:[10,0.65,8],color:PAL[4],type:"solid",level:4,deco:true});
  z-=10;
  // Second bounce
  makeBox(scene,objects,{pos:[0,y,z],size:[7,0.55,7],color:"#a855f7",emissive:"#5b21b6",type:"bounce",level:4,bounce:13.5});
  z-=9; y+=2.9;
  makeBox(scene,objects,{pos:[0,y,z],size:[10,0.65,8],color:PAL[0],type:"solid",level:4});
  z-=10;
  cpBox(scene,objects,[0,y,z],5,"BOING COMPLETE! GOOD BOY/GIRL");
  z-=12;
  ...
  z-=36+8; y+=0.3;
  cpBox(scene,objects,[0,y,z],6,"LAVA SURVIVED! RAWR XD!!!!");
  ...
  for (var i=0;i<6;i++) {
    ...
    if (i%2===0) makeCylinder(scene,hazards,{pos:[bx+2.2,y+0.95,z],radius:0.44,height:1.9,color:"#ff006e",level:6,spin:{axis:"y",speed:3.5}});
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],7,"BRIDGES DONE! NYA NYA NYA!");
  z-=12;
  ...
  makeTailSpinner(scene,hazards,{center:[-3,y+0.34,z],armLength:7.5,speed:1.9,level:7,color:"#ff3b9d"});
  makeTailSpinner(scene,hazards,{center:[3,y+0.34,z],armLength:7.5,speed:-2.1,level:7,color:"#a855f7"});
  makeSign(scene,"DOUBLE TAIL ATTACK!!!",0,y+4,z-5,5.2,1.1);
  z-=16; y+=0.3;
  ...
  x=0;
  cpBox(scene,objects,[x,y,z],8,"TWO TAILS BEATEN!! WOOF!!");
  z-=12;
  ...
  for (var i=0;i<5;i++) {
    ...
    makeSphere(scene,hazards,{pos:[i%2===0?-5:0,y+1.2,z],radius:0.8,color:"#ff4fc3",level:8,
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],9,"ORBS DODGED! *HOWLS*");
  ...
  for (var i=0;i<4;i++) {
    makeBox(scene,objects,{pos:[0,y,z],size:[7,0.6,6],color:"#ffb07c",type:"solid",level:9,
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],10,"LAVA + MOVING?? U INSANE!!!");
  z-=12;
  ...
  makeTailSpinner(scene,hazards,{center:[-4.5,y+0.34,z],armLength:8,speed:2.1,level:10,color:"#ff4fc3"});
  makeTailSpinner(scene,hazards,{center:[4.5,y+0.34,z],armLength:8,speed:-2.3,level:10,color:"#9ee7ff"});
  makeTailSpinner(scene,hazards,{center:[0,y+0.34,z-5.5],armLength:7,speed:2.6,level:10,color:"#b8ff9e"});
  makeSign(scene,"TRIPLE TAIL TERROR!!!",0,y+4,z-7,5.5,1.1);
  z-=18; y+=0.4;
  ...
  x=0;
  cpBox(scene,objects,[x,y,z],11,"SECTION 10 CLEAR! HOWL!!!");
  z-=12;
  ...
  for (var i=0;i<7;i++) {
    ...
    makeSphere(scene,hazards,{pos:[x+(i%2===0?3:-3),y+1.1,z],radius:0.65,color:"#a855f7",level:11,
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],12,"TINY PAWS BIG W!!! RAWR!");
  z-=12;

  // ──────────────────────────────────────────────────────────────────────────
  // SECTION 12 – big bounce + spinner on landing.
  makeSign(scene,"SECTION 12: MEGA BOUNCE UWU",0,y+3.2,z+2,5.8,1.2);
  makeBox(scene,objects,{pos:[0,y,z],size:[8,0.55,8],color:"#a855f7",emissive:"#5b21b6",type:"bounce",level:12,bounce:15});
  makeSign(scene,"MEGA BOINGY TIME OwO",0,y+3,z-3.5,5,1.1);
  z-=10; y+=3.8;
  makeBox(scene,objects,{pos:[0,y,z],size:[13,0.65,11],color:"#ff9de6",type:"solid",level:12});
  makeTailSpinner(scene,hazards,{center:[0,y+0.34,z],armLength:9,speed:1.9,level:12,color:"#ff3b9d"});
  z-=13; y+=0.3;
  ...
  for (var i=0;i<7;i++) {
    ...
    if (i===2||i===5) makeTailSpinner(scene,hazards,{center:[sx13,y+0.28,z],armLength:5,speed:2.1+i*0.1,level:13,color:PAL[(i+3)%8]});
    z-=5.5;
  }
  makeBox(scene,objects,{pos:[0,y,z],size:[10,0.65,8],color:PAL[1],type:"solid",level:13});
  z-=10;
  cpBox(scene,objects,[0,y,z],14,"STAIRWAY TO UWU HEAVEN!");
  z-=12;
  ...
  z-=10;
  cpBox(scene,objects,[0,y,z],15,"WAVY WAVE CHECKPOINT!! OWO");
  z-=12;
  ...
  x=mazeEndGx*grid;
  y+=0.3;
  cpBox(scene,objects,[x,y,z],16,"MAZE CONQUERED! FURSONA WIN!!");
  makeFursuitHead(scene,x-4,y+1.1,z,"#ff9de6");
  makeFursuitHead(scene,x+4,y+1.1,z,"#9ee7ff");
  x=0; z-=12;
  ...
  for (var i=0;i<6;i++) {
    ...
    makeCylinder(scene,hazards,{pos:[0,y+1,z+(i%2===0?2.2:-2.2)],radius:0.48,height:2.2,color:"#ff006e",level:16,spin:{axis:"y",speed:3.8+i*0.25}});
    z-=8; y+=0.09;
  }
  makeBox(scene,objects,{pos:[0,y,z],size:[10,0.65,8],color:PAL[2],type:"solid",level:16});
  z-=10;
  cpBox(scene,objects,[0,y,z],17,"CYLINDERS CAN'T STOP U!! OWO");
  z-=12;
  ...
  for (var si=0;si<sp17.length;si++) {
    makeTailSpinner(scene,hazards,{center:[sp17[si][0],y+0.34,z+sp17[si][1]],armLength:6,
      speed:(si%2===0?2.4:-2.6)+si*0.08,level:17,color:PAL[si]});
  }
  makeSign(scene,"FOUR TAILS. NO MERCY. UWU",0,y+4.5,z-7,6,1.1);
  z-=20; y+=0.4;
  ...
  x=0;
  cpBox(scene,objects,[x,y,z],18,"FOUR TAILS?? FOUR WINS!!! W");
  z-=12;
  ...
  for (var i=0;i<5;i++) {
    ...
    makeSphere(scene,hazards,{pos:[-5,y+1.2,z],radius:0.8,color:"#ff4fc3",level:18,
  ...
  makeBox(scene,objects,{pos:[x,y,z],size:[8,0.55,8],color:"#a855f7",emissive:"#5b21b6",type:"bounce",level:19,bounce:16});
  ...
  makeSign(scene,"U ESCAPED THE UWU DIMENSION!!",0,y+5.5,z-7,8,1.6);
  makeSign(scene,"CONGRATUWULATIONS!! *NUZZLES*",0,y+3.6,z-5.5,7,1.3);
  for (var i=0;i<7;i++) makeFursuitHead(scene,(i-3)*3.5,y+1.1,z,PAL[i%8]);
  for (var i=0;i<5;i++) makePawPrint(scene,(i-2)*5,y+0.44,z-6,i*0.7);
}
...
export default function FurryObbyGame({onBack}) {
  ...
  useEffect(function(){
    ...
    scene.background=new THREE.Color("#c89dff"); // vivid purple sky
    scene.fog=new THREE.Fog("#c89dff",35,220);
    ...
    scene.add(new THREE.HemisphereLight("#ffe8fa","#9e5bbd",1.0));
    ...
    scene.add(sun);
    var fill=new THREE.DirectionalLight("#ffb8f8",0.45); fill.position.set(-15,10,15); scene.add(fill);
    ...
    var sky=new THREE.Mesh(new THREE.SphereGeometry(380,32,18),
      new THREE.MeshBasicMaterial({color:"#e0a0ff",side:THREE.BackSide}));
    scene.add(sky);

    // Fluffy fur-ball clouds
    var clouds=[];
    var cloudColors=["#fff","#ffecff","#ffe8fc","#e8f8ff"];
    ...
    function saveCheckpoint(platform) {
      ...
      showToast("Checkpoint "+lvl+"/"+TOTAL_SECTIONS+" — NYA~!");
      triggerFlash("check");
